...
These source observations were then checked against a live Coolify instance, first 4.3.18 and then 4.3.23; section 10 records the supported baseline, what changed between those releases, the sanitized fixtures, and the behavior that only the live server revealed.
...
│   ├── gitinfo/
│   │   └── gitinfo.go              # Origin remote and branch through git; https and SSH remote forms; anonymous reachability probe
│   ├── sshkey/
│   │   └── sshkey.go               # Ed25519 deploy key pairs in OpenSSH format, never written to disk
│   ├── envfile/
...
│   │   ├── buildpack.go            # Build pack detection, per-pack flags, Coolify's defaults
...
| `alias` | Adds and removes a second name for the running binary (`coolship alias`): a symlink, or a copy on Windows, in the binary's own directory and nowhere else. Refuses names that already run another command on `PATH` and files that are not Coolship, identified from Go build information without running them. The executable path, PATH lookup, and OS are injected. `cmd` calls it directly, since it touches no project, credentials, or server. | None |
...
Project, environment, and application strings are names, not strings guessed to be UUIDs. Optional `project_uuid`, `environment_uuid`, and `application_uuid` fields provide identity pins. `link` (and `init`, which links) always writes all three beside the names, so later commands read the pinned resources directly. Pins contain identifiers, not secrets. While a pin resolves, the UUID is authoritative and a renamed resource is reported as label drift. When a pinned UUID is missing in its scope, resolution falls back to the name beside it and warns once on stderr (`coolship.toml pins <kind> <uuid> that no longer exists; found <name> by name. Run coolship link to refresh.`); a missing or ambiguous name, or a missing pin with no name, is still an error. Resolution never rewrites the file. Name-only files, as earlier versions wrote them, keep working unchanged.
...
- An environment override that names another environment drops the environment and application pins, which name resources in the bound environment, and resolves the application by name there; the project pin is kept. A binding whose application is pinned without a name cannot be overridden that way and is rejected with an actionable error, instead of retaining an old binding or deploying to a different application silently.
...
Resolve credentials once per invocation, retain them only inside authentication/client objects, and exclude them from context serialization and diagnostics. `link` does not change the Coolify CLI default or create a second credential database. `login` is the one writer: it checks the URL against Coolify's public `GET /api/health` (no token sent) before the token is asked for, refuses a URL and token a saved context already holds, and verifies the token against `GET /version` and `GET /teams/current` before saving. In a terminal those steps run as one form in `internal/ui` (a checklist on `Steps` with the answers listed above the active question, Shift+Tab back, and Retry, Go back, or Leave on a failed check); the service exposes them as `CheckInstance`, `CheckLogin`, and `SaveLogin`, and `Login` runs all three for noninteractive use. It round-trips fields it does not know (Coolify CLI's `lastUpdateCheckTime`, anything on other instances), makes the first instance the default, and writes atomically with the same permissions Coolify CLI uses (directory 0750, file 0600). The token is read without echo or from stdin, never from a flag.

### Local state

The MVP can resolve fresh on each command. Add `.coolship/state.json` only when avoiding repeated reads has demonstrated value. (The per-user `state.json` next to `preferences.toml` is unrelated: `internal/update` keeps the time of the last release check there, and the latest release it saw, for the daily update notice the executable prints.) Reserve it for schema-versioned, disposable UUID mappings; ignore `.coolship/` in Git when introducing it.
...
A name is matched in its parent's list: `GET /projects`, then the environments `GET /projects/{uuid}` embeds (or `GET /projects/{uuid}/environments`), then the applications in the environment details. A pinned UUID is read directly instead: `GET /projects/{uuid}`, the project-scoped `GET /projects/{uuid}/{environment_uuid}`, and `GET /applications/{uuid}`, where a 404 is the pin going missing. Each read starts as soon as its UUID is known, so a fully pinned binding is one round of concurrent requests, and a caller can start a read that needs only the application UUID beside the application read (`status` reads its history that way). Results are examined parent first, so the reported failure is the highest one in the hierarchy, and reads still running when resolution fails are cancelled. Verify the returned identities and application membership. Pins bypass name matching, but not scope or membership checks: the environment read is scoped by the project, and a pinned application must appear in the environment's list before its own read is used. A pin that fails those checks, or answers 404, is missing: its level is resolved by name as if unpinned (listing projects, or the project's environments, as needed), the application read and any read started beside it are restarted with the found UUID, and one warning names every missing pin. Follow supported pagination when listing resources; incomplete candidate lists must not become successful unique matches. Convert read failures into typed errors instead of pretending the candidate list is empty.
...
| `init` | Read the repository's remote and branch, settle the build pack (`service/buildpack.go`: `--build-pack`, else the pack named by `--dockerfile` or `--compose-file`/`--compose-domain`, else detection in Coolify's order; per-pack flags refused when they do not belong, Coolify's defaults filled in), refuse a linked directory before any request, confirm the plan, create the application without deploying, then persist and verify the binding through `link`'s write step. `--deploy` reuses the deployment service on the verified session. The source is settled first: an anonymous `git ls-remote` through the injected `gitinfo` function tells a public remote from a private one; a private one is created through a GitHub App, checked against the app's branch listing of the repository before creation (one call that also tells whether the app can reach it), or a deploy key, with the remote in SSH form — each chosen by name or prompt, never implicitly; `--create-deploy-key` generates an Ed25519 pair in `sshkey`, registers the private half, returns the public half, and stops before the application. | `GET /servers`, the `link` reads, optionally `POST /projects`; for a private source `GET /github-apps`, `GET /github-apps/{id}/repositories/{owner}/{repo}/branches`, or `GET /security/keys` and optionally `POST /security/keys`; then `POST /applications/public`, `/applications/private-github-app` with `github_app_uuid`, or `/applications/private-deploy-key` with `private_key_uuid`, all with `instant_deploy: false` and the pack's fields (`dockerfile_location`, `docker_compose_location`, `docker_compose_domains`, `is_static`, `publish_directory`, the three commands, `health_check_enabled: false` for a Dockerfile); with `--deploy`, `POST /deploy` and `GET /deployments/{uuid}`. |
...
| `delete` | Resolve once, then delete the application after a confirmation that names its status and the URLs it answers on; the plan is read from the server because nothing undoes the deletion. The server deletes the application at once and queues the rest, so there is nothing to observe and no retry. Its own defaults remove the configurations, connected networks, and images that only the deleted application owned; `--keep-volumes` is the one choice exposed, since it is the one that loses data. The local binding is kept unless `--unlink` says otherwise, and a binding that cannot then be removed is a warning, never the command's failure: the application is already gone. Deleting a project, environment, or server stays out of scope. | `DELETE /applications/{uuid}` with `delete_volumes=false` only when volumes are kept; the server requires a team administrator's token. |
...
| `domain`, `domain set` | Show the application's domains; replace them after confirmation, then read the application back to report what the server kept. A Compose application has `fqdn` null and its domains per service in `docker_compose_domains`, a JSON object keyed by service name inside a JSON string, which `models.ComposeDomains` decodes in document order; `domain` lists them per service, `status`, `open`, and a finished deployment use the first web URL across services, and `domain set` takes `SERVICE=URL` pairs and replaces the map whole, refusing a plain URL once the application is read and before any change is sent. Coolify keeps a URL's port apart from the domain, in `domain_port_overrides` (present since at least 4.3.18, first observed on 4.3.21), as the container port the domain routes to, and reports the domain without it, for a Compose service as for any other application; the read back therefore accepts a requested URL kept without its port, and a URL with a port is never a no-op, since the port kept is not read. | Shared resolution; `PATCH /applications/{uuid}` with `domains`, `redirect`, `force_domain_override`, or `docker_compose_domains` (an array of `name`, `domain`, `redirect`) for a Compose application, which the server refuses `domains` for. |
| `login`, `logout` | Verify and store an instance in the Coolify CLI configuration; remove one. | `GET /api/health` (no token), `GET /version`, `GET /teams/current`; no resolution. |
...
`internal/problem` is the catalog of failures Coolship knows how to explain. Each entry has a stable snake_case code, an owner (`coolify` when the fix is in Coolify's setup, `coolship` otherwise), a fixed hint, a documentation link (Coolify's own page for Coolify setup problems, a heading on `docs/content/docs/platform/errors.mdx` otherwise), and the fix Coolship could offer (log in, log in again, link, pick a context, or open a Coolify page on the instance). `problem.Classify` walks the chain with `errors.Is` and `errors.As` over the typed errors of `coolify` (`HTTPError` with its `Denial` and `Abilities`, `RequestError.Kind`, `NotRunningError`, `ProtocolError`, `UncertainSubmissionError`), `auth`, `project`, `config`, and `resolver`, plus the deployment timeout through a method, so it never imports `service` and `service` can use it. When a lower layer's text already carries the advice the hint gives, the message drops it, so the advice is said once. `internal/coolify` reads a 403 body on every method because Coolify's `ApiAllowed` and `ApiAbility` middleware answer with fixed sentences (`API is disabled.`, `You are not allowed to access the API.`, `Missing required permissions: …`, and the team member role refusal); any other 403 body of a read is still dropped.

The executable boundary (`ui.ReportError`) prints a catalogued failure as `Error [code]: message`, then `Hint:` and `Docs:` lines, on stderr. A failure without an entry keeps its single `Error:` line, because a generic code in text would be noise on every flag typo. With `--format json` the boundary also writes `{"error":{"code","message","hint","docs_url"}}` on its own line to stdout, after anything the command printed, with a generic code (`invalid_input`, `unclassified`, `checks_failed`, `interrupted`, `cancelled`) when the catalog has none; a child process's status prints nothing in either format. Exit codes do not depend on the catalog. A test keeps one heading per code on the errors page.

The executable runs the tree through `cmd.Execute`, which owns the one offer to fix a failure. When `problem.Classify` names a fix, output is human, the streams are interactive, and the `hints` preference is on, it prints the failure itself, puts the fix to the user, and carries it out by running `login`, `link`, or `init` in a fresh command tree built from the same dependencies, or by opening the instance page through the injected browser launcher. A saved context is applied by rerunning with `--context`. The failed command then runs once more in another fresh tree with the same arguments, after a question when it changes something on the server. A declined or unfinished fix returns the original error wrapped by `ui.Reported`, which keeps its exit code and tells `ui.ReportError` it is already on screen; the rerun's own failure is returned as is, so there is never a second offer. Every tree of one invocation shares one buffered stdin reader, so a line one prompt reads ahead is not lost to the next.

Next-step hints after `link` and `init` come from `service.ProjectState`, one more resolution plus the variables and the deployment history read together, and are chosen in `ui.NextSteps`. The read only happens when the hints would be shown; if it fails, the plain deploy hint stands in.

`init` and `link` draw their whole run in a terminal as one block behind a left bar, `ui.Block`: the command sets `Streams.Block` from `ui.NewBlock`, which is nil unless output is human and a live view may be drawn (an interactive stderr terminal at normal verbosity), and the prompter, `Steps`, the warnings, the hints, and the result line all write through it, so a nil block is the text every other run always had. The bar is the border of the focused field in the pickers' Huh theme, taken through Lip Gloss so the two cannot drift. Long lines wrap inside the bar; rows of the live `Steps` view are prefixed before they are cut to the width, so a frame stays one terminal line per row and `eraseView` still counts rows. Inside a block the result is brief, one `<application> is linked` line, because the plan and the checklist already said the rest; a piped stdout, JSON, and verbose runs keep the full result. `InitPlan.Private` and `InitPlan.Directory` exist for that plan only and are left out of its JSON.

Verbose output is one line per request attempt on stderr: method, URL, status, time taken, and retry number. Identical successful exchanges in a row (same method, URL, and status, as a deployment poll produces) are written once and summarized as `×N more (last D)` when a different exchange arrives or the executable flushes the trace at exit; retries and exchanges without a response are never folded, and debug writes every exchange. Debug output adds the request and response headers and bodies, since issue #18 asks for full requests and responses. The client masks the Authorization header to the token's last four characters before the trace sees it, so the raw token never leaves the client. Bodies are redacted by the client before the trace sees them, because a debug trace is the thing people paste into bug reports and CI logs, and gh, flyctl, and Wrangler redact theirs for the same reason. In a JSON body, the value of every field named `value` or `real_value`, or whose name contains `secret`, `password`, `private_key`, `token`, or `api_key` (but not a `_uuid` or `_id` reference to one), is replaced with `[redacted]`, containers included, and the body is re-encoded compactly in its original order; empty and null values stay visible. That covers variable values (`env set`, `env push`, `env pull`), the private key `init --create-deploy-key` registers, and the secrets of GitHub Apps and applications. A body that is not JSON, such as an HTML error page, is shown as received; one that starts like JSON but does not parse, such as a cut-short refusal, is withheld with a note. Setting `COOLSHIP_DEBUG_UNREDACTED=1` shows bodies as sent and received, for the rare bug that lives in a value; the token stays masked either way. Control characters are removed in both cases. Redaction is by field name and cannot see a secret inside an unrelated field, such as a compose file or a build log, so debug output still goes to stderr for the one run, is never cached or written to a file, and the documentation still warns against keeping it in shared CI logs. Variable diff output masks values by default. Raw application/build logs can themselves contain secrets: treat them as requested log output and do not also persist them in a debug cache.
...
Migration rules live in `project.Propose`, which both `link` and the writer use: adding a new named target keeps the others and needs no review; changing an existing target's binding needs review, unless only its UUID pins change, which adds or refreshes pins for the resources the file already names; converting between the single and named forms drops the other form's bindings and therefore needs review, which the prompt says explicitly. `link --target NAME` proposes the working directory relative to the configuration root as the root, so linking from `apps/web` records `apps/web`. An unchanged file is never rewritten, and a file that changed since discovery is a conflict.
...
**Verified: Coolify 4.3.23** (2026-09-19), reached through Cloudflare, with a token holding read, write, deploy, and sensitive-read abilities. The baseline was first established on 4.3.18 (2026-09-09/10, recorded below) and re-verified when the instance reached 4.3.23: `scripts/e2e` passed all 21 steps, and `init` creating a public-repository application followed by `delete` removing it again closed the two paths `scripts/e2e` does not cover. Three things were not re-run: `init` through a GitHub App and through a deploy key, `preview` deploying a real webhook-created preview, and linking a two-target monorepo. Neither private source nor a preview can be set up through the API — the example application holds no preview record any more, so only the refusal path was exercised. Coolify's source is unchanged between the two releases for the endpoints behind all three.

The comparison was made against upstream tags `v4.3.18` and `v4.3.23` (the reference checkout is a fork at `424dbd3`, roughly 4.3.19). `DeployController`, `ProjectController`, `SecurityController`, `GithubAppController`, `ApplicationDeploymentStatus`, `EnvironmentVariable`, and `app/Policies/` are byte-identical between the two, so deployment submission, observation, cancellation, history, hierarchy resolution, variables, private keys, and GitHub App lookups behave exactly as section 1 and the 4.3.18 observations below describe. Four changes touch endpoints Coolship uses, none of them breaking:

- **`GET /applications/{uuid}/logs` accepts `lines=all`**, with `-1` as a compatibility alias, where any value below 1 previously fell back to the default 100 (`normalizeLogLines` in `bootstrap/helpers/docker.php`). Coolship refuses anything outside 1–10000 before sending, so it never reaches either form and its behaviour is unchanged; exposing "all" would be a new choice, not a fix. The handler also gained `$this->authorize('view', $application)`, and `ApplicationPolicy::view` checks only team membership, which `ownedByCurrentTeamAPI` had already scoped the lookup to — no observable change for a token-owned application.
- **`GET /applications/{uuid}/previews/{pull_request_id}/logs` is new**: the same handler, keyed additionally by an `ApplicationPreview`, answering `422 Invalid pull_request_id.` for a non-numeric or non-positive id and `404 Preview not found.` when the application holds no such preview. This is the endpoint that would let `logs` read a preview's runtime logs, which section 9 records as impossible; it is not used yet.
- **`POST /applications/public` normalizes an scp-style remote.** `create_application` now converts `user@host:owner/repo` to its https form before parsing and stores that form, so a GitHub SSH remote is recognized as `github.com` and reduced to the `owner/repo` slug on the public GitHub source (`source_id` 0) instead of being kept verbatim as an unparseable URL. Coolship sends an https URL to this endpoint and the SSH form only to `/applications/private-deploy-key`, whose handling is unchanged, so nothing changes for `init`. The GitHub App path extracts its slug through the shared `gitRepositorySlug` helper, accepting the same inputs as before.
- **Domain conflict detection was rewritten** (`checkDomainUsage`, `checkIfDomainIsAlreadyUsedViaAPI`, and the new `domainConflictKey` and `applicationDomainEntries` in `bootstrap/helpers/domains.php`): a conflict is keyed by the domain without its scheme or trailing slash, and a Compose application's per-service domains are included in the comparison. This is the refusal `domain set` relays, so its wording and the cases that trigger it come from this code.

Two further changes are visible but inert here: `Application` and `ApplicationPreview` gained a `max_restart_count` default attribute, which appears in application documents and is ignored by Coolship's structs, and the storage endpoints dropped `host_path`, which Coolship does not call. `domain_port_overrides` is **not** new in this range — it is already present in 4.3.18; the behaviour recorded for it below was simply first observed while the instance ran 4.3.21.
...
- **Private sources.** `GET /github-apps` lists the team's apps and system-wide ones, the built-in `Public GitHub` first (`id` 0, `is_public: true`); `client_secret` and `webhook_secret` are withheld without the sensitive ability. `GET /github-apps/{id}/repositories` and `.../repositories/{owner}/{repo}/branches` are keyed by the row `id`, not the uuid, and only for apps of the token's team — a system-wide app of another team answers `404 GitHub app not found`. The repository listing merges every installation page (up to 100 pages of 100, fetched one after another with 20 s per GitHub call) into GitHub's own objects, so its size and duration grow with the installation and large ones time out — the reason Coolify's own creation path checks access with `GET /repos/{slug}` instead — and Coolship does not call it. The branch listing is a single `GET /repos/{owner}/{repo}/branches` without paging, so it relays GitHub's first page only (30 branches, alphabetical; `coollabsio/coolify` and `rails/rails` both have `main` beyond it) and relays GitHub's 404 for a repository the app cannot see. Coolship uses it as the access check, refuses a missing branch only when the page is not full, and otherwise warns that the branch could not be checked. `POST /applications/private-github-app` takes the app's `uuid` as `github_app_uuid`, accepts `git_repository` as an https URL or `owner/repo` and stores the slug with `source_type` `GithubApp`, `source_id` the row id, and `repository_project_id` from GitHub; it verifies access itself with `GET /repos/{slug}` and answers `404 Repository not found or not accessible by the GitHub App.` otherwise. `GET /security/keys` lists `id`, `uuid`, `name`, `description`, `public_key`, `fingerprint`, and `is_git_related` (the keys of GitHub Apps), plus `private_key` with the sensitive ability, which Coolship never decodes; the instance's own key is `id` 0, and keys Coolify made for servers are listed like deploy keys, which is why a key is never chosen implicitly. `POST /security/keys` takes `name`, `description`, and `private_key` (PEM text, or base64 of it), validates it, derives the public key and fingerprint, refuses a fingerprint it already holds with `422 Private key already exists.`, and answers `201 {uuid}`; an unencrypted OpenSSH Ed25519 key from `sshkey` was accepted and the derived `public_key` matched the generated line. `POST /applications/private-deploy-key` takes `private_key_uuid` and validates `git_repository` with `ValidGitRepositoryUrl`, which accepts scp-style SSH (`git@host:owner/repo.git`, `git@host:2222/owner/repo.git`) and http(s) but not `ssh://`; the application keeps the remote verbatim with `private_key_id` and no source, and the deployment job clones it with `GIT_SSH_COMMAND="ssh -i <key> …"` whatever its form, so an https remote would be cloned anonymously — Coolship always sends the SSH form. `DELETE /security/keys/{uuid}` answers `422 Private Key is in use and cannot be deleted.` while an application, server, or Git integration uses the key, and 200 once none does. Verified by creating `coolship-init-gh-test` through an installed GitHub App and `coolship-init-key-test` through a key `init` generated and registered on the example repository (deployed to `finished` in 35 s; the build log redacts the `user@host` part of the remote), then deleting both applications, the key, and the repository's deploy key.
- **An anonymous `git ls-remote --heads`** distinguishes the two cases quickly: GitHub answers a private repository with `fatal: could not read Username for 'https://github.com': terminal prompts disabled` in under half a second and a public one with its heads in about a second. Coolship runs it in an empty temporary directory that is also `HOME` and the ceiling of repository discovery, with `GIT_TERMINAL_PROMPT=0`, no askpass, no credential helper, `http.extraHeader` reset, the global and system Git configuration disabled, and every `GIT_*` variable except trust settings and tracing dropped along with `SSH_AUTH_SOCK`: git's HTTP transport reads `~/.netrc` through libcurl whatever Git's own settings say, and `GIT_DIR` or `GIT_CONFIG_PARAMETERS` would bring a checkout's configuration along, so nothing stored on the machine can make a private repository look public. On Unix the command runs in its own process group, which a cancellation kills whole, and it carries a `WaitDelay` everywhere: a killed `git` alone leaves `git-remote-https` holding the pipes and the connection until its own timeout, which held a cancelled probe for about two minutes before.
