...
| `stop` | Resolve once; leave an application that is not `running` alone with a warning; confirm, request the stop, then poll the status until it leaves `running` or the timeout passes, reporting the last status seen. | `POST /applications/{uuid}/stop`, then `GET /applications/{uuid}` while waiting. |
