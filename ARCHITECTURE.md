...
| `ui` | Owns prompts, terminal capability checks, colors, progress rendering, JSON, and error presentation. The only package that imports the Charm stack (Bubble Tea, Huh, Lip Gloss; ADR 0001), enforced by a test; the stack receives each stream's colour decision as an explicit profile and never decides for itself. May consume service result/event types; never fetches data or selects a remote target by business rules. | `service` |
