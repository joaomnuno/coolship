...
Verbose output is one line per request attempt on stderr: method, URL, status, time taken, and retry number. Debug output adds the request and response headers and bodies, since issue #18 asks for full requests and responses. The client masks the Authorization header to the token's last four characters before the trace sees it, so the raw token never leaves the client. Bodies are shown as sent and received, with control characters removed, so debug output can carry variable values (`env set`, `env push`, `env pull`) and any other secret a request or response holds. It goes to stderr for the one run and is never cached or written to a file, and the documentation warns against keeping it in shared CI logs. Variable diff output masks values by default. Raw application/build logs can themselves contain secrets: treat them as requested log output and do not also persist them in a debug cache.
