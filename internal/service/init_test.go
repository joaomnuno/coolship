...
func TestInitSettlesBuildPack(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"dockerfile flag infers the pack", []string{"deploy/Dockerfile.prod"}, BuildOptions{Dockerfile: "deploy/Dockerfile.prod", Port: 8080}, InitPlan{BuildPack: "dockerfile", Port: 8080, Dockerfile: "/deploy/Dockerfile.prod"}, models.ApplicationSpec{BuildPack: "dockerfile", PortsExposes: "8080", DockerfileLocation: "/deploy/Dockerfile.prod", HealthCheckEnabled: &off}, "", ""},
		{"compose file flag infers the pack", []string{"deploy/stack.yml"}, BuildOptions{ComposeFile: "deploy/stack.yml", ComposeDomains: []ComposeDomain{web}}, InitPlan{BuildPack: "dockercompose", ComposeFile: "/deploy/stack.yml", ComposeDomains: []ComposeDomain{web}}, models.ApplicationSpec{BuildPack: "dockercompose", PortsExposes: "80", DockerComposeLocation: "/deploy/stack.yml", DockerComposeDomains: []models.ComposeDomain{{Name: "web", Domain: "https://web.example.com"}}}, "", ""},
		{"compose domain flag infers the pack", []string{"compose.yml"}, BuildOptions{ComposeDomains: []ComposeDomain{web}}, InitPlan{BuildPack: "dockercompose", ComposeFile: "/compose.yml", ComposeDomains: []ComposeDomain{web}}, models.ApplicationSpec{BuildPack: "dockercompose", PortsExposes: "80", DockerComposeLocation: "/compose.yml", DockerComposeDomains: []models.ComposeDomain{{Name: "web", Domain: "https://web.example.com"}}}, "", ""},
		{"compose domain without a compose file", []string{"stack.yml"}, BuildOptions{ComposeDomains: []ComposeDomain{web}}, InitPlan{}, models.ApplicationSpec{}, "", "--compose-file PATH"},
		{"dockerfile and compose flags conflict", []string{"Dockerfile", "compose.yml"}, BuildOptions{Dockerfile: "Dockerfile", ComposeFile: "compose.yml"}, InitPlan{}, models.ApplicationSpec{}, "", "name different build packs"},
		...
		{"dockerfile flag on explicit railpack", []string{"Dockerfile"}, BuildOptions{BuildPack: "railpack", Dockerfile: "Dockerfile"}, InitPlan{}, models.ApplicationSpec{}, "", "--dockerfile applies"},
		...
		{"compose domain infers compose over a detected dockerfile", []string{"Dockerfile"}, BuildOptions{ComposeDomains: []ComposeDomain{web}}, InitPlan{}, models.ApplicationSpec{}, "", "--compose-file PATH"},
		{"compose flags on explicit dockerfile", []string{"Dockerfile"}, BuildOptions{BuildPack: "dockerfile", ComposeDomains: []ComposeDomain{web}}, InitPlan{}, models.ApplicationSpec{}, "", "--compose-domain apply"},
