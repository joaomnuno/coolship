...
var catalog = map[Code]entry{
	...
	CodeTokenExceedsRole: {owner: OwnerCoolify, docs: coolifyPermissions, fix: FixOpenURL, page: apiTokens,
		hint: "Team members may only hold read-only tokens. Ask a team admin for a higher role, or create a token with read permissions only under Keys & Tokens, API tokens, then run coolship login."},
