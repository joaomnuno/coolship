...
func TestPaletteIsNoOpWhenOff(t *testing.T) {
	...
	if got := on.key("Status"); got != "\x1b[2mStatus:\x1b[0m" {
		t.Fatalf("key = %q", got)
	}
}
...
func TestStyledOutputEqualsPlainOutputWithoutColor(t *testing.T) {
	...
	wantStyled := "\x1b[32m[ok]\x1b[0m   Configuration: coolship.toml\n" +
		"\x1b[33m[warn]\x1b[0m Binding: renamed\n" +
		"\x1b[1;31m[FAIL]\x1b[0m Server: unreachable\n" +
		"\x1b[2m[skip]\x1b[0m Application\n" +
		"[odd] Other\n"
...
func TestStylesWrapSanitizedText(t *testing.T) {
	...
	if got, want := diagnostic.String(), "Deployment d1: \x1b[1;32mfinished\x1b[0m\n"; got != want {
	...
	if got, want := diagnostic.String(), "\x1b[33mWarning:\x1b[0m name\\tchanged\n"; got != want {
...
func TestKeysAndDiffMarkersAreStyledOnStdoutOnly(t *testing.T) {
	...
	if !strings.HasPrefix(out.String(), "\x1b[2mApplication:\x1b[0m web (app-1)\n") || !strings.Contains(out.String(), "\x1b[2mStatus:\x1b[0m running\n") {
	...
	if got, want := out.String(), "\x1b[2mConfiguration:\x1b[0m    /p/coolship.toml\n\x1b[2mTarget:\x1b[0m           api\n"; got != want {
	...
	want := "Comparing .env with runtime variables of web\n" +
		"\x1b[32m+\x1b[0m A=********  (local only; push creates it)\n" +
		"\x1b[33m~\x1b[0m B: local ********, remote ********\n" +
		"\x1b[31m-\x1b[0m C=********  (remote only; push --prune deletes it)\n" +
		"\x1b[2m?\x1b[0m D  (remote value withheld; cannot compare)\n" +
		"1 unchanged\n"
	if out.String() != want {
...
func TestErrorsAndPromptsAreStyledOnStderr(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{errors.New("server returned 500\n"), "\x1b[1;31mError:\x1b[0m server returned 500\\n\n"},
		{context.Canceled, "\x1b[2mInterrupted\x1b[0m\n"},
		{service.ErrCancelled, "\x1b[2mCancelled\x1b[0m\n"},
	} {
		diagnostic.Reset()
	...
	want := "\x1b[1mSelect project:\x1b[0m\n  \x1b[36m1.\x1b[0m one (p1)\n  \x1b[36m2.\x1b[0m two\\n (p2)\n\x1b[1mChoice [1-2, q to cancel]:\x1b[0m "
	...
	if !strings.HasPrefix(diagnostic.String(), "\x1b[1mDelete /p/coolship.toml?\x1b[0m\n") || !strings.HasSuffix(diagnostic.String(), "\x1b[1mConfirm [y/N]:\x1b[0m ") {
