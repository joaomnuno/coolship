...
- Keep discovery and local writes in `internal/project`, the TOML schema in `internal/config`, credentials in `internal/auth`, and HTTP to Coolify in `internal/coolify`. The one exception is `internal/update`, which owns the single unauthenticated GitHub release request behind the update notice and its per-user `state.json` next to `preferences.toml`, with the HTTP client and URL injected. `internal/preferences` owns `preferences.toml`, including the one write behind `config set` and the config form, and its `Keys()` list is the single description of every key. `internal/alias` owns the one write next to the running binary behind `coolship alias`, and `cmd` calls it directly because it touches no project, credentials, or server.
...
- Keep credential files read-only. Never persist tokens or fetched secret values in project configuration, fixtures, or Git. Debug output (`--debug`) goes to stderr only, masks the token to its last four characters, and redacts the values of secret-bearing JSON fields in request and response bodies unless `COOLSHIP_DEBUG_UNREDACTED=1` is set, as `ARCHITECTURE.md` section 8 records; never write it to a file or cache.
