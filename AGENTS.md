...
- Keep discovery and local writes in `internal/project`, the TOML schema in `internal/config`, credentials in `internal/auth`, and HTTP to Coolify in `internal/coolify`. The one exception is `internal/update`, which owns the single unauthenticated GitHub release request behind the update notice and its per-user `state.json` next to `preferences.toml`, with the HTTP client and URL injected.
