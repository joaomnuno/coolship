...
Coolship is in early development. `init`, `link`, `status`, `deploy`, `logs`, `open`, `unlink`, `delete`, `config`, `doctor`, `env pull|diff|push`, `preview`, `dev`, `domain`, and `login` are implemented, tested, and verified end to end against a live **Coolify 4.3.23** instance. [Server compatibility](/docs/concepts/server-compatibility) describes what that does and does not cover, and [Limits](/docs/platform/limits) lists what the API does not allow.
