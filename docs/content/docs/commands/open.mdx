...
`0` when the URL was resolved (whether or not a browser was launched); `1` when the application has no domain to open or the server cannot be reached; `2` when the directory is not linked.
