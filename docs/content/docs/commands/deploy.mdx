...
`--logs` and `--no-logs` cannot be combined. Without either, the `build_logs` key of the [preferences file](/docs/commands/config) decides; without that, the log stays collapsed. Piped output and `--format json` always stream the log.
...
- An observation that stops before the server's verdict — `--timeout` elapsing, an interrupt, a lost connection — ends the checklist with `✗ Observation stopped`, not `Deployment failed`; the deployment continues on the server.
