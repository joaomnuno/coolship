...
When Coolify already holds an image for the same commit and nothing that affects the build changed, it skips the build and the checklist says so. Only that line marks a stage skipped; a stage with no marker in the log, for instance because the token may not read the build log, stays dim and unmarked:

```text
✓ Deployed coolship-example to production in 0:23
...
A Compose application, an application with ports mapped to the host, and a preview start the new container without a rolling update. Coolify writes `Rolling update started.` before it starts a container, so a container that starts, or old containers removed, with no rolling update before it settles that row: it reads `not used`, drawn like a skipped stage, and the container and cleanup rows stand on their own rather than under it:

```text
✓ Deployed api to production in 0:31
  ✓ build                       0:12
...
Rows are cut to the terminal's width, so a narrow terminal never wraps one. When the checklist ends, the rows it drew are erased and the final checklist is printed once in their place, where it stays.
...
`url` is the last line of the human output and `url_kind` says what it is: `application` for the application's URL (the first service's, for a Docker Compose application), `deployment` for its page in Coolify (`<instance>/project/<project>/environment/<environment>/application/<application>/deployment/<uuid>`). With `--no-wait`, `status` is `queued` and the URL is the deployment page. A failed or cancelled deployment is reported with its state, the deployment page, and a nonzero exit. In JSON mode no progress reaches stdout.
