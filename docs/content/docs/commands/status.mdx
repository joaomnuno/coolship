...
`0` on a successful read regardless of the application's status; `1` when the server cannot be reached or refuses the request; `2` when the directory is not linked or the binding no longer resolves.
