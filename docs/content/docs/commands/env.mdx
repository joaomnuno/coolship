...
In a terminal, the push is a checklist of two steps, drawn like the deploy checklist: `Compare variables`, which names how many keys it would create, update, or delete, and `Write variables`, which names the keys it wrote. The confirmation appears below the finished comparison. Details name keys only, never values. Piped output, `--format json`, and `--verbose` runs print no checklist.
