...
`init` is the first step for a repository that is not on Coolify yet. It reads the `origin` remote and the checked-out branch from Git (SSH forms are normalized to `https://github.com/owner/repo`), detects the build pack from the application root, shows the plan, and creates the application only after you confirm. Nothing is deployed unless you pass `--deploy`, or answer `y` to `Deploy now? [y/N]`, which a terminal asks afterwards along with the [next steps](/docs/platform/output#hints) for the new application.
...
A warning, such as a Compose application without a domain, is printed once, above the plan, where it can still change the answer. The bar is a plain character with `NO_COLOR` or `--no-color`, and on a narrow terminal long values wrap inside it.
...
| `dockercompose` | Runs the compose file; each service publishes the ports the file gives it, so `--port` is refused.         | `--compose-file PATH` names another file. `--compose-domain SERVICE=URL`, repeatable, gives a service its domain; without one, the plan warns and you set them afterwards with `domain set SERVICE=URL`. |
...
- **A Compose application's domains are per service.** `init` sets them with `--compose-domain`; afterwards [`domain`](/docs/commands/domain) lists them per service and `domain set SERVICE=URL` replaces them. Variables work as for any application.
