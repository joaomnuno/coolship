...
The keys are read and checked today and take effect once the verbosity flags and the collapsed build log land. Credentials never go in this file: the Coolify URL and token stay in [Coolify CLI's file](/docs/concepts/credentials), so one login serves both tools.
