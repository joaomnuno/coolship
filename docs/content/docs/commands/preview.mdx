...
In a terminal the deployment is shown as the same stage checklist as [`deploy`](/docs/commands/deploy), build log collapsed. `--logs` and `--no-logs` cannot be combined; without either, the `build_logs` preference decides, and without that the log stays collapsed. Piped output and `--format json` are unchanged.
