...
- **Compose domains are per service.** `init` sets them with `--compose-domain` and `domain set SERVICE=URL` replaces the whole set; Coolify keeps only the services its copy of the compose file defines, which it reloads from the repository on a deployment or in its UI, and the API cannot clear the set.
...
- **Names are matched exactly** within their parent; an ambiguous or missing selection is an error, never a guess. A pinned UUID that disappears falls back to the name beside it, with a warning to run `coolship link`.
