...
Create a token in Coolify under *Keys & Tokens* with the read, write, and deploy abilities. `login` checks the URL through Coolify's public health check (`GET /api/health`) before asking for the token, then verifies the token against the server (`GET /version` and the current team) before saving them. A URL and token that a saved context already holds are refused as a duplicate. The token is read without echo, or from stdin with `--token-stdin`; it is never accepted as a flag, so it stays out of shell history and process listings.
