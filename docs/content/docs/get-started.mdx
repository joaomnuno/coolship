...
`NO_COLOR` or `TERM=dumb` drops the colour and keeps the banner. Piped output, such as a CI log, and `--quiet` print a single `Installed` line instead. [`coolship alias`](/docs/commands/alias) adds `cs` as a short name for `coolship`.
