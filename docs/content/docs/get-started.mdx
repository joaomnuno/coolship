...
<Callout title="init needs a public repository">
  Coolify clones it without credentials. A private repository needs a GitHub App or deploy key registered in Coolify first, which the API takes by UUID; create those applications in Coolify and run `link`. Docker Compose projects are refused by `init` as well.
</Callout>
