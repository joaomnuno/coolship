...
In a terminal the script ends with the Coolship wordmark, the installed version and path, and what to run next:
...
```text
  ...
  Next:      coolship login
  Optional:  coolship alias  (adds cs)
```

`NO_COLOR` drops the colour. Piped output, such as a CI log, and `--quiet` print a single `Installed` line instead. [`coolship alias`](/docs/commands/alias) adds `cs` as a short name for `coolship`.
